...
import { Plus, Upload, Send, DollarSign, Package, Clock, CheckCircle, User, X, FileText } from 'lucide-react';
...
const Index = () => {
  ...
  const submitInquiry = async () => {
    ...
    try {
      ...
      const productsData = newInquiry.products
        .filter(p => p.type && p.quantity)
        .map(p => ({
          type: p.type,
          quantity: parseInt(p.quantity) || 0,
          unit: getDefaultUnit(p.type),
          specifications: p.thickness,
          remarks: p.remarks
        }));
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          project_name: newInquiry.projectName,
          delivery_address: newInquiry.deliveryAddress,
          expected_date: newInquiry.expectedDate || null,
          needs_transport: newInquiry.needsTransport,
  ...
  const getDefaultUnit = (productType) => {
    switch (productType) {
      case 'Steel Be ams': return 'pieces';
  ...
  const pendingCommissions = orders
    .filter(order => !order.commission_paid && order.status === 'Completed')
    .reduce((sum, order) => sum + (order.commission || 0), 0);
                    ...
                    inquiries.map(inquiry => (
                      <div key={inquiry.id} className="bg-white rounded-xl shadow-md p-6 border border-gray-200 hover:shadow-lg transition-shadow">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h3 className="text-xl font-semibold text-gray-900 mb-1">{inquiry.project_name}</h3>
                            <p className="text-sm text-gray-600">ID: {inquiry.inquiry_number} • {new Date(inquiry.created_at).toLocaleDateString()}</p>
                          </div>
                        ...
                        <div className="space-y-2 mb-6">
                          {inquiry.products && inquiry.products.map((product, idx) => (
                        ...
                        {inquiry.status === 'Quoted' && inquiry.total_amount > 0 && (
                          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            <div className="flex justify-between items-center mb-4">
                              <span className="font-medium text-gray-900">Total Amount:</span>
                              <span className="text-2xl font-bold text-blue-600">${inquiry.total_amount.toLocaleString()}</span>
                            </div>
                ...
                <div className="space-y-4">
                  {orders.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>No orders yet. Orders will appear here once inquiries are accepted.</p>
                    </div>
                  ) : (
                    orders.map(order => (
                      <div key={order.id} className="bg-white rounded-xl shadow-md p-6 border border-gray-200 hover:shadow-lg transition-shadow">
                        <div className="flex justify-between items-start mb-6">
                          <div>
                            <h3 className="text-xl font-semibold text-gray-900 mb-1">{order.project_name}</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                          ...
                          <div className="bg-green-50 rounded-lg p-4">
                            ...
                            <p className="text-2xl font-bold text-green-600">${(order.paid_amount || 0).toLocaleString()}</p>
                          </div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-medium text-green-600 flex items-center">
                              ...
                              {(order.paid_amount || 0) >= order.total_amount ? 'Fully Paid' : 'Pending Payment'}
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="divide-y divide-gray-200">
                    {orders.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p>No commissions yet. Complete orders to earn commissions.</p>
                      </div>
                    ) : (
                      orders.map(order => (
                        <div key={order.id} className="px-6 py-6 hover:bg-gray-50 transition-colors">
                          <div className="flex justify-between items-center">
                            <div>
                              <p className="font-semibold text-gray-900 text-lg">{order.project_name}</p>
                              <p className="text-sm text-gray-600 mt-1">{order.order_number} • Order Total: ${order.total_amount.toLocaleString()}</p>
                            </div>
                            <div className="text-right">
                              <p className="font-bold text-gray-900 text-xl">${(order.commission || 0).toLocaleString()}</p>
